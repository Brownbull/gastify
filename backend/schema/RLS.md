...
| `group_stat_tombstones` | Group scope (read under the analytics scope-swap GUC) | Via erasure / group-leave-delete (under the group GUC) | DENY | DENY | P16 Phase 1 group-stat void (D82); group-scope-bound; **append-only at the DB** — split `group_stat_tombstones_select` (FOR SELECT) + `group_stat_tombstones_insert` (FOR INSERT) policies, no UPDATE/DELETE policy → denied (migration 036, hardening 035's `FOR ALL` policy); 027 fail-safe GUC form |
