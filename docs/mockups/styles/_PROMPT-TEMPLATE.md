...
12. **🔒 Brand-invariant wordmark** — the `gastify` wordmark ALWAYS uses `font-family: 'Baloo 2', cursive; font-weight: 700; font-size: 24px;` across every theme, mode, and platform. Only the color changes (bound to `var(--primary)` per theme). Every style prompt inherits this rule — do not override.
