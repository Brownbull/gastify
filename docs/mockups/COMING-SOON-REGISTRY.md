...
| CS-21 | Historial (Items) | Vista agregada por producto (1 fila/producto + total gastado + N compras expandibles + "N productos · $total") | The `/items` list is per-occurrence (one row per line item); there is no aggregated-by-product endpoint, and the list response carries only cursor/has_more (no total match count / total spend) | ✅ Shipped as the per-occurrence list re-skinned to the design-lab row grammar (per-category icon tile + CategoryChip + merchant·date meta + amount); the "N productos" strip shows the LOADED count | Needs `GET /api/v1/items/aggregated` (per-product total_spent + txn_count + child purchases) → HistoryItemRow with the expandable PurchaseList + real "N productos · $total" totals |
| CS-22 | Historial (Items) | Vínculo Gustify (link a line item ↔ a Gustify ingredient) | Cross-app integration: no gustify mapping field on items, no gustify-icons asset set in web, no link flow | ✅ omitted (the row's left tile shows the item's own category icon instead of a Gustify link slot) | Needs a gustify mapping on items + the gustify-icons asset dir + the LinkSourcePopup/AddItemSheet cross-app flow |
