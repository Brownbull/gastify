...
const SEEDS = [
  { localId: "a", label: "a.jpg" },
  { localId: "b", label: "b.jpg" },
];
...
describe("batchScanStore", () => {
  ...
  it("stages and consumes batch inputs once (hand-off to BatchReview)", () => {
    const inputs: BatchScanInput[] = [
      { localId: "a", label: "Happy", source: { kind: "testCase", caseId: "happy" } },
    ];
    stageBatchInputs(inputs);
    expect(consumeBatchInputs()).toEqual(inputs);
    expect(consumeBatchInputs()).toEqual([]); // cleared after consumption
  });
});
