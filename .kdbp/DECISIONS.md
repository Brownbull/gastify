...
## D95 — Leave-delete proof lives in the leaver's PERSONAL scope; tombstones CASCADE with their group (2026-06-11)

**Decision.** Two coupled changes after the two-user e2e found `DELETE /groups/{id}` 500ing on any group that had ever been left with `delete_shared=true`: (1) the `dsr_group_leave_delete` audit event is now written to the **leaver's personal scope** (was: the group scope) — it is the LEAVER's Art-17-adjacent proof, it becomes visible in their own `/consent/audit` trail (it wasn't before), and it survives a later group deletion; the group-scoped form FK-blocked `ownership_scopes` deletion forever, and the P78 append-only trigger makes such rows immovable by the app. (2) `group_stat_tombstones.ownership_scope_id` → **ON DELETE CASCADE** (migration 041): the app role deliberately has no DELETE policy (036 append-only), and referential actions are RLS-exempt — so the FK cascade is the single sanctioned removal path, firing only when the whole group (whose stats the tombstones existed to void) is deleted. RLS-ordering note: `leave_group` now flushes the group-scoped writes under the group GUC BEFORE swapping to personal for the audit insert (the erasure-R1 bug class). **Data repair:** staging-e2e's 5 legacy group-scoped rows re-homed via a superuser one-shot (append-only trigger disabled inside one transaction, re-enabled); production verified to have ZERO legacy rows and zero tombstones — fix-forward only. **Status:** accepted. **Affects:** `backend/app/api/groups.py` (leave_group), `backend/app/models/group_stat_tombstone.py`, `backend/alembic/versions/041_tombstone_scope_cascade.py`, `backend/tests/test_group_leave_void.py`.

## D96 — Credit/tier model: Free = 20 scan credits/mo, no statements, no batch; Premium = CLP 5.000/mo, 60 credits, 3 statements, 3 batch (2026-06-12)

**Decision (user direction, 2026-06-12).** Two tiers, monthly quotas, **no rollover** (quota resets each month, unused credits do not accumulate):
- **Free:** 20 single-receipt scan credits per month. Credit-card STATEMENT scanning NOT available. Batch scanning NOT available (when batch ships as a gated feature).
- **Premium:** CLP $5.000/month → 60 single-receipt scan credits, 3 statement scans/month, 3 batch scans/month.

**Why.** Statement extraction calls real Gemini in production and was entirely un-gated (the rate-limit analysis's top finding); tier-quota gating is the product answer — free users simply lack the feature, premium gets a small monthly allowance priced into the subscription. Receipt scans were already credit-gated atomically (P16 P4); this fixes the monthly numbers and adds the reset/no-rollover semantics. The PAYMENT PROVIDER (how CLP 5.000 is charged — Webpay/MercadoPago/etc.) remains undecided; quota ENFORCEMENT does not depend on it (free-tier enforcement works immediately; premium can be a manually-set flag until payments ship). **Status:** accepted. **Affects:** billing service (monthly reset job, no-rollover), users/billing schema (tier + per-feature monthly counters), statements + scans + future-batch endpoints (tier checks), pricing/credit UX (web+mobile), RATE-LIMIT-PLAN.md (statement/batch items superseded by tier gating).

## D97 — Production-direct: production is the sole deployed environment + source of truth for all testing, with disposable data (2026-06-25)

**Decision (user direction, 2026-06-25).** From now until explicitly decided otherwise, **production is the single deployed environment** and the source of truth for all deployed and end-to-end testing. Production **data is disposable** — seed, destroy, and recreate freely. The `staging` and `staging-e2e` Railway environments were deleted (cost control during the local/mockup phase; Railway project was ~$7.89/mo, ~95% memory, with 3 always-on Postgres) and are NOT maintained. The promotion ladder (local → staging-e2e → staging → production) in `docs/runbooks/ENVIRONMENTS.md` is **suspended, not deleted**.

**Why.** During the mockup phase the staging lanes ran 24/7 (3 Postgres + 4 API/web) but produced no value — deployed/e2e proof can run directly on production now that there are no real users and data is throwaway. Dropping the duplicate environments cuts spend and removes the staging-restore + dual-config maintenance burden. Postgres cannot sleep on Railway, so deleting the environments (not just `railway down` of stateless services) was required to zero their cost.

**Consequences.** (1) Gabe's "Review after staging" becomes **review after production** — deploy candidate to prod, capture deployed proof there, then `/gabe-review`. (2) The production web is provisioned (reuses the `gastify-staging` Firebase project; email/password test-user login behind `VITE_PROD_TEST_AUTH_ENABLED`, default off) — see `docs/runbooks/PRODUCTION-TEST-USER.md`. (3) The disposable-data stance holds ONLY while there are no real users — revisit before any real launch (the retention cron + DSR machinery still run in production). (4) Restore staging via `docs/runbooks/RAILWAY-STAGING-TEARDOWN.md` when the gated model is reinstated.
