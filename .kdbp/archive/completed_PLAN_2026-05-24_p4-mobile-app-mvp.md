...
Deliver a cross-platform mobile app MVP — Expo/React Native scaffold, Firebase auth, native camera receipt scan, WebSocket scan progress, transaction ledger/edit, platform sign-out isolation, and push registration — proving the first mobile user journey on Android physical hardware for the current roadmap cycle. iOS runtime testing is officially deferred until after the roadmap is implemented.
...
| 5 | Mobile E2E journey + edge tests | `core-only, native-mobile, test` | Jest + Maestro coverage for sign in → camera scan → stream → transaction view → edit → sign out → assert keystore/cache eviction on Android physical hardware, plus key edge cases. iOS runtime testing is deferred post-roadmap. | ent | high | ✅ | ✅ | ✅ | ✅ |
...
- **Key deliverables:**
  ...
  - Android platform configuration for native permissions; iOS configuration may remain, but runtime proof is deferred post-roadmap
...
- **Key deliverables:**
  ...
  - Maestro E2E configuration for Android physical hardware
  ...
  - CI-compatible command documentation for the Android/S23 gate; iOS simulator/device testing is tracked as a post-roadmap deferred lane
- **Exit signal per ROADMAP:** Mobile E2E journey green on Android physical hardware.
- **Trade-offs accepted:** See D42 and D47
...
- Phase 5 depends on Phases 1-4 and must validate the current ROADMAP exit signal on Android physical hardware. iOS runtime testing is deferred until after the roadmap is implemented.
...
| iOS runtime testing is intentionally deferred beyond the current roadmap | low | Track the deferred lane in P31/D47; do not block Phase 5 on local `xcrun` availability |
...
- Tier distribution: ent × 5. No MVP phases because native auth storage, camera capture, streaming recovery, sign-out isolation, and Android hardware E2E are all exit-signal behavior for the current roadmap cycle.
- Open PENDING items carried into this plan: P8/P9 mobile scan review metadata gap, P18 streaming middleware risk, P31 deferred iOS runtime lane.
- Testing ladder: current Phase 1 gate is fast JS checks + staging Firebase auth verification; next gate is a Samsung S23 physical-device smoke with an EAS Android APK; Firebase Test Lab becomes later Android compatibility evidence; Phase 5 requires Maestro scripted journeys on Android hardware. iOS runtime testing is deferred post-roadmap.
...
- **Date:** 2026-05-24
- **Reason:** Goal achieved — all 5 phases complete (5/5 ✅ across Exec, Review, Commit, Push). Android/S23 runtime gate passed; iOS runtime testing is deferred post-roadmap by D47/P31.
