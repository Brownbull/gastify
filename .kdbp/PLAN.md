...
| 2 | W2 · App shell + navigation | Port AppScaffold/Nav → 4-tab BottomNav (Inicio·Compras·Gastos·Perfil) + ScanFab (DM-5) + AppHeader + framed surface, adapted to TanStack Router (tabs = `<Link>`); replace `AppLayout`; resolve the 11→4 IA (homes for statements/groups/notifications/reports/trends/items inside tabs/Perfil); port shell atoms. | mvp | med | ⬜ | ⬜ | ⬜ | ⬜ |
...
Phase 1: W1 · Token foundation (tier mvp)
