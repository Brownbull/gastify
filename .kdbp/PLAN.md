...
- **(C, 2026-06-10) The go/no-go MUST resolve the open DSR/void residuals — fix-or-accept, never silent.** For each of **P69** (void leaks through gravity-baseline averages + partial quarter/week series buckets), **P70** (no live-PG erasure-flow regression test in CI), **P71** (admin-removal forecloses a removed member's erasure recourse for their group shares), **P72** (portability omits the user's group-shared copies — GDPR Art 20): the checklist records EITHER a fix (with its proof) OR an explicit accepted-residual with the per-jurisdiction rationale. A self-attestation (D88) that hides a known gap is not defensible.
