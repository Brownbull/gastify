...
- **Created:** 2026-04-23
- **Last Updated:** 2026-04-27 (L1 🎯 complete; Current Phase advanced to L2; P3/P4/L0/Spike P14 retro-ticked Commit+Push since they shipped in be9aefd+P5 — Review ⬜ remains as honest signal that no /gabe-review pass occurred, tracked in PENDING.md P11)
...
| Spike P15.0 | `/gabe-mockup validate` mode + per-screen validator | Codify `/gabe-mockup validate` mode in gabe_lens (parallel to `spike`). Calibrated against gastify's hybrid architecture (tweaks.js + per-device suffixes): 28 base screens × 3 viewports + 3 desktop-only files = 87 validation specs running C1 overflow / C2 narrow-cols / C3 empty-content / C4 KDBP-rule checks. 0 active findings on gastify. Templates emitted to tests/mockups/validate/ + .kdbp/MOCKUP-VALIDATION.md live document + docs/mockups/VALIDATE-MODE-RECIPE.md. | spike, mockup-validate | mvp | low | ✅ | ⬜ | ✅ | ✅ |
...
- **2026-04-27 — Phase L2a batch 1 Exec 🔄 (mockups-legacy molecules):** First batch of 3 molecules extracted via /gabe-mockup auto-mode dispatch. **banner** (atom-composing, multi-variant: info/warning/error/offline-edge-bleed) — sourced from `features/batch-review/views/BatchCaptureCreditSection.tsx` + `features/scan/components/ScanError.tsx` + `hooks/app/useOnlineStatus.ts`; consumes the L1 button atom (btn--primary / btn--ghost / btn--icon). **state-tabs** (canonical primitive) — sourced from `features/items/components/ItemViewToggle.tsx` (the React port's most-reused multi-state pattern: pill-shape with sliding background indicator + `role="tablist"`); also drives ChartModeToggle, DrillDownModeToggle, InsightsCarousel, ReportCarousel; primitive (no atom dependencies). Two visual modes shipped: `.state-tabs` (canonical pill) + `.state-tabs--simple` (flat-row variant for low-fi screens). **toast-system** (system layer) — wraps L1 toast atom with positioning (`.toast-stack` fixed bottom-center above nav), exit animation (`.is-leaving`), and the `useToast` hook contract (single-toast semantics, 3s auto-dismiss, errors 6s — explicitly diverges from the spike P14.0 React-port toast which had a queue + max-3 + FIFO eviction; the live frontend's `shared/hooks/useToast.ts` deliberately stays single-toast). Drift-vs-clean-slate documented per molecule (BEM-lite class names matching L1 atom convention, frontend-token vocabulary `--text-primary`/`--bg-tertiary` instead of clean-slate's `--ink`/`--surface-2`). `assets/css/molecules.css` populated 17→237 lines, 3 sections, zero hex/rgb literals. `molecules/index.html` sub-hub flipped placeholder→live with catalog cards + inline previews per molecule (real DOM, not iframe). Atoms `button.html` + `toast.html` got their "Used by molecules" placeholder backrefs upgraded to live cross-ref entries — closes the bidirectional contract for these two atoms. Principal hub Molecules card flipped to `data-status="live"`, meta-pill updated. Root `INDEX.md §3` populated with 3 catalog rows; phase-status table shows L2a 🔄. Phase L2 Exec ⬜→🔄. Remaining ~15 L2a molecules queued (cards / modals / sheet / drawer / navs / FAB / filters / form / list-item) — to be batched in subsequent /gabe-mockup runs.
