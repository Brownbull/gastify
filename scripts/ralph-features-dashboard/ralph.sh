...
echo "Starting Ralph (dashboard) — Tool: $TOOL — Max iterations: $MAX_ITERATIONS — Cost cap: \$$MAX_USD_PER_BATCH/batch"
...
for i in $(seq 1 $MAX_ITERATIONS); do
  echo ""
  echo "==============================================================="
  echo "  Ralph Iteration $i of $MAX_ITERATIONS ($TOOL) — dashboard"
  echo "==============================================================="
  ...
  if echo "$OUTPUT" | grep -q "<promise>COMPLETE</promise>"; then
    echo ""
    echo "Ralph (dashboard) completed all tasks at iteration $i!"
    exit 0
  fi
...
echo ""
echo "Ralph (dashboard) reached max iterations ($MAX_ITERATIONS)."
