...
echo "Starting Ralph (history) — Tool: $TOOL — Max iterations: $MAX_ITERATIONS — Cost cap: \$$MAX_USD_PER_BATCH/batch"
...
for i in $(seq 1 $MAX_ITERATIONS); do
  echo ""
  echo "==============================================================="
  echo "  Ralph Iteration $i of $MAX_ITERATIONS ($TOOL) — history"
  echo "==============================================================="
  ...
  if echo "$OUTPUT" | grep -q "<promise>COMPLETE</promise>"; then
    echo ""
    echo "Ralph (history) completed all tasks at iteration $i!"
    exit 0
  fi
...
echo ""
echo "Ralph (history) reached max iterations ($MAX_ITERATIONS)."
