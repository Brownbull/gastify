 ...
 * strong without a loud full-color card. Title extrabold; body font-medium muted.
 *
 * Vendored from design-lab/src/design-system/molecules/StatusCard.tsx (D102).
 * Used for over-budget warnings, scan results, sync notices, and home tips.
 */
...
export function StatusCard({ tone = "info", title, children, icon, className = "" }: StatusCardProps) {
  return (
    <div
      className={`flex gap-gt-12 rounded-gt-2xl border-2 border-gt-line-strong p-gt-14 shadow-gt-sm ${toneSurface[tone]} ${className}`}
    >
      <span
        className={`grid h-9 w-9 shrink-0 place-items-center rounded-gt-pill border-2 border-gt-line-strong ${toneDisc[tone]}`}
      >
